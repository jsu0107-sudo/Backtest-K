...
4. 첫 화면 사전 계산과 단일 파일판을 재생성하고 전체 회귀 테스트를 실행한다.
5. `data/` 또는 정적 생성물이 바뀌면 기본 브랜치에 커밋·푸시한다.
6. GitHub와 연결된 Vercel 프로젝트가 새 정적 파일을 배포한다.
...
- 예약 작업에서는 공식 API가 시간 초과 등으로 일시 중단되면 `status=unavailable`을 기록하고 월 수익률 게시는 계속한다. 프런트는 이를 **공식시세 대사 일시 확인 불가**로 표시하며, 다음 평일 실행에서 다시 대사한다.
