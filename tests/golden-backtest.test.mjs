...
// 로딩 규칙 회귀 감시: 동결 입력과 라이브 data/의 해당 구간이 같아야 한다.
// 여기만 깨지면 코드가 아니라 데이터가 개정된 것이므로, 확인 후 픽스처를 갱신한다.
test("라이브 data/와 동결 입력이 일치 (데이터 개정 감시)", () => {
  for (const [id, table] of Object.entries(golden.inputReturns)) {
    ...
    for (const [month, value] of Object.entries(table)) {
      assert.equal(live.get(month), value, `${id} ${month} 수익률이 개정됨 (코드 문제가 아님)`);
    }
  }
});
